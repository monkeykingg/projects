 ...
 */
int read_from(int client_index, struct sockname *usernames, fd_set *fs, int max_fd) {
    int fd = usernames[client_index].sock_fd;
    char buf[BUF_SIZE + 1];

    // init a temp for modification
    char temp[BUF_SIZE + 1];

    int num_read = read(fd, &temp, BUF_SIZE);
    temp[num_read] = '\0';

    // check if there is a new user occur
    if (usernames[client_index].username == NULL) {
        usernames[client_index].username = malloc(strlen(buf) + 1);
        strcpy(usernames[client_index].username, temp);
        usernames[client_index].username[strlen(usernames[client_index].username)-1] = '\0';
        strcpy(buf, "Welcome, new user! \n");
    } else {
    ...
    for (int i = 4; i <= max_fd; i++) {
        if (FD_ISSET(i, fs)) {
            if (num_read == 0 || write(i, buf, strlen(buf)) != strlen(buf)) {
    ...
    while (1) {
        ...
        for (int index = 0; index < MAX_CONNECTIONS; index++) {
            if (usernames[index].sock_fd > -1 && FD_ISSET(usernames[index].sock_fd, &listen_fds)) {
                ...
                int client_closed = read_from(index, usernames, &all_fds, max_fd);
                if (client_closed > 0) {
