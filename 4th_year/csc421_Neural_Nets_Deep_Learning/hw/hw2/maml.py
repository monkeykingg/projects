...
def gd_step(cost, params, lrate):
    ...
    ### YOUR CODE HERE
    result = {}
    cost_grad_function = ag.grad(cost)
    result_params = cost_grad_function(params)

    for i in params:
        new = params[i] - lrate * result_params[i]
        result[i] = new
...
class MetaObjective:
    ...
    def __call__(self, params, return_traj=False):
        ...
        ### YOUR CODE HERE
        cost_function = InnerObjective(self.x, self.y)
        latest_params = trajectory[-1]
        lrate = self.inner_lrate
        steps = self.num_steps

        for i in range(steps):
            new = gd_step(cost_function, latest_params, lrate)
            trajectory.append(new)
            latest_params = trajectory[-1]

        final_cost = cost_function(latest_params)
        ### END CODE
...
def train():
    ...
    for i in range(OUTER_STEPS):
        ...
        cost = MetaObjective(x_train, y_train, INNER_LRATE, INNER_STEPS)
