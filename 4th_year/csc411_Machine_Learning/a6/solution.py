...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        ndata, ncomp = R.shape
        return (R.sum(0) + self.prior.a_mix - 1.) / (ndata + ncomp * (self.prior.a_mix - 1.))
    ...
    def update_theta(self, X, R):
        ...
        denom = (R.sum(0) + self.prior.a_pixels + self.prior.b_pixels - 2.)
        return (np.dot(R.T, X) + self.prior.a_pixels - 1.) / denom.reshape((-1, 1))
    ...
    def posterior_predictive_distribution(self, X, M):
        """Compute the posterior predictive distribution for unobserved pixels given the observed
    ...
    def visualize_predictions(self, X, M, title=None):
        ...
        P = self.posterior_predictive_distribution(X, M)
        imgs = np.where(M, X, P)
...
def print_part_2_values():
    ...
    P = model.posterior_predictive_distribution(X, M)
