...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        ######################## Your code here #########################
        a = self.prior.a_mix
        numerator = np.sum(R.T, axis=1) + a - 1
        denominator = R.shape[1] * (a - 1) + np.sum(R)
    ...
    def update_theta(self, X, R):
        ...
        numerator = np.dot(R.T, X) + a - 1
        denominator = np.dot(R.T, X) + np.dot(R.T, 1 - X) + a + b - 2
        return np.divide(numerator, denominator)
        #################################################################
    ...
    def compute_posterior(self, X, M=None):
        ...
        log_p_x_given_z = np.dot(M * X, np.log(theta).T) + \
                          np.dot(M * (1 - X), np.log(1 - theta).T)
        log_p_z_x = log_p_x_given_z + np.log(pi)
        #################################################################
